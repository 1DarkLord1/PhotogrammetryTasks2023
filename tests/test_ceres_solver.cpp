...
TEST (CeresSolver, HelloWorld1) {
    ...
    // DONE 1: почему результирующая производная не ноль? мы ведь должны были сойтись в минимуме функции 0.5*(10-x)^2
    // Производная берется у невязки f(x) = 10 - x, а она константа -1 на всём промежутке.
...
TEST (CeresSolver, HelloWorld2) {
    ...
    for (int d = 0; d < 3; ++d) {
        ...
        // DONE 3: раскомментируйте^, почему он находит не то что ожидалось?
        // residuals[0] в ResidualToParaboloid инициализировалось неправильно.
    ...
    // DO 4: если любопытно и хватит времени - можете попросить ceres-solver посчитать якобианы в некоторых точках подобно тому как это делалось в конце теста HelloWorld1
...
public:
    ...
    bool operator()(const T* const line, T* residual) const {
        ...
        // DONE 5 посчитайте единственную невязку - расстояние от нашей точки-замера до текущего состояния прямой (для извлечения корня, помня про T=Jet, нужно использовать ceres::sqrt):
...
void evaluateLineFitting(double sigma, double &fitted_inliers_fraction, double &mean_inliers_distance, double outliers_fraction=0.0, bool use_huber=false) {
    ...
    if (outliers_fraction > 0.0 && !use_huber) {
        threshold *= 35.0; // ослабляем порог если есть выбросы и мы к ним не устойчивы (не робастны за счет loss-функции (функции потерь) Huber-а)
    }
    ...
    for (int d = 0; d < 3; ++d) {
        ...
        // DONE 7 расскоментируйте сверку найденной прямой и эталонной
        ...
        // DONE 7 поправьте тест так или иначе (хотя бы пост-процессингом)
    }

    // Оцениваем качество идеальной прямой
    double inliers_fraction, mse;
    evaluateLine(points, ideal_line, sigma, inliers_fraction, mse);
    ASSERT_GT(inliers_fraction, 0.99 * (1 - outliers_fraction)); // DONE 8 раскоментируйте, почему эта проверка падает? как поправить?
    // Проверка падала, потому что вычислялась доля инлаеров среди всех точек, в которые в том числе входили искуственно
    // созданные случайные выбросы. Необходимо посчитать долю инлаеров среди точек, которые не являются выбросами.
    ASSERT_LT(mse, 1.1 * sigma * sigma); // DONE 9 раскомментируйте, почему проверка падает? на каких тестах она падает, на каких проходит? попробуйте отладить рассчет mse_inliers_distance в evaluateLine
    // Проверка падала на тестах  FitLineNoiseAndOutliers и FitLineNoiseAndOutliersWithHuberLoss. Это происходило из-за
    // того, что в строчке "if (abs(dist) <= 3 * sigma) {" в функции evaluateLine расстояние сравнивалось без модуля,
    // а оно вычисляется со знаком, и получалось, что куча точек с отрицательным расстоянием становились инлаерами.
    ...
    if (outliers_fraction == 0 || use_huber) {
        // DONE 10 раскоментируйте обе проверки, почему они падают? в каких тестах? поправьте (в т.ч. подобно тому как было с ослаблением порога выше)
        // Проверки падали на тесте FitLineNoiseAndOutliersWithHuberLoss. use_huber было выставлено в 1, но outliers_fraction != 0,
        // поэтому проверки падали по той же причине, что и самая первая проверка.
