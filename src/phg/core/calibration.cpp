...
cv::Vec3d phg::Calibration::project(const cv::Vec3d &point) const
{
    ...
    // DONE 11: добавьте учет радиальных искажений (k1_, k2_) (после деления на Z, но до умножения на f)

    double r_squared = x * x + y * y;
    double c = 1 + k1_ * r_squared + k2_ * r_squared * r_squared;
    x *= c;
    y *= c;
...
cv::Vec3d phg::Calibration::unproject(const cv::Vec2d &pixel) const
{
    ...
    // DONE 12: добавьте учет радиальных искажений, когда реализуете - подумайте: почему строго говоря это - не симметричная формула формуле из project? (но лишь приближение)
