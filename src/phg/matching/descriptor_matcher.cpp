...
void phg::DescriptorMatcher::filterMatchesRatioTest(const std::vector<std::vector<cv::DMatch>> &matches,
    ...
    for (auto &match: matches) {
        if (match.size() == 1 || match[0].distance / match[1].distance < K_RATIO) {
...
void phg::DescriptorMatcher::filterMatchesClusters(const std::vector<cv::DMatch> &matches,
    ...
    for (int i = 0; i < n_matches; i++) {
        std::set<int> indices2_train, indices2_query;
        for (int j = 0; j < total_neighbours; j++) {
            if (distances2_query.at<float>(i, j) < radius2_query) {
                indices2_query.insert(indices_query.at<int>(i, j));
            }
        }
        for (int j = 0; j < total_neighbours; j++) {
            if (distances2_train.at<float>(i, j) < radius2_train) {
                indices2_train.insert(indices_train.at<int>(i, j));
            }
        }

        std::vector<int> intersection;
        std::set_intersection(indices2_query.begin(), indices2_query.end(), indices2_train.begin(),
                              indices2_train.end(), std::back_inserter(intersection));
